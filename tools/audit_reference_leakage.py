...
Implements step 1 of creative-director-ai issue #17. Read-only: opens the
senpai-reel DuckDB, compares every generated output against the competitor
message units it actually cited, and reports verbatim n-gram overlap.
...
def load_rows(db_path: str):
    """Return (gen_id, content_type, output_text, [(unit_id, unit_text), ...]) per generation."""
    import duckdb
...
def audit(rows, n: int):
    findings = []
    counted = 0
    for gen_id, ctype, output, cited in rows:
        if not cited:
            continue  # nothing cited means nothing to leak from
        counted += 1
...
def main() -> int:
    ...
    ap.add_argument("--n", type=int, default=5, help="n-gram length (default 5)")
    ...
    except Exception as exc:  # missing file, missing table, locked db
    ...
    for f in findings[: args.show]:
        print(f"  [{f['content_type']}] {f['gen_id']}  <- unit {f['unit_id']}")
        for o in f["overlaps"][:4]:
            print(f'      "{o}"')
        print(f"      reference: {f['unit_text'][:110]}")
        print()
    ...
    if args.json:
        with open(args.json, "w") as fh:
            json.dump({"n": args.n, "generations": len(rows), "with_references": counted,
                       "affected": len(affected), "findings": findings}, fh, indent=2)
